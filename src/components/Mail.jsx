...
function Mail() {
    ...
    return(
    <div className="mail">
        <input className={`input ${emailerr ? 'input__error' : ''}`} type="email" id="mail" name="Mail" value={email} onChange={(e)=> {setEmail(e.target.value);validate(); console.log(e.target.value)}} placeholder="Type your email address here…" />
        
        <button className={`mail__button ${emailerr ? 'mail__button__error' : ''}`}>
            <div>
         ...
         <div className={`mail__text ${emailerr ? 'mail__text__visible' : 'mail__text__hidden'}`}>Your email is invalid</div> 
    </div>
    );
}
